...
uint256 hashGenesisBlock("0xd46d2182a003dabb4cf6035b6e80ad5cd2c82d6b93a80df6dd1bd3d1d6e2ba98");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Biebercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Biebercoin Signed Message:\n";
...
{
    // Biebercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Biebercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Biebercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Biebercoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd46d2182a003dabb4cf6035b6e80ad5cd2c82d6b93a80df6dd1bd3d1d6e2ba98");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1401355912;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084697226;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BiebercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BiebercoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Biebercoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BiebercoinMiner terminated\n");
        throw;
    }
}
